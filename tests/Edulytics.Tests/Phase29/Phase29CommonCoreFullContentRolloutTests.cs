...
{
    [Fact]
    public void CommonCoreFullRolloutLocksExact1466PublishedCanonicalTargets()
    {
        ...
        Assert.Equal(1466, actualRows.Length);
        Assert.Equal(1466, actualRows.Select(x => x.LessonCode).Distinct(StringComparer.Ordinal).Count());
        ...
        foreach (var item in expected)
        {
            ...
            Assert.Equal(2, lesson.Translations.Count);
            Assert.Equal(new[] { "en", "pl" }, lesson.Translations.Select(x => x.CultureCode).OrderBy(x => x).ToArray());
            var en = lesson.Translations.Single(x => x.CultureCode == "en");
            var pl = lesson.Translations.Single(x => x.CultureCode == "pl");
            Assert.NotEqual(en.Title, pl.Title);

            foreach (var translation in lesson.Translations)
            {
                Assert.True(translation.Explanation.Length >= 260, $"{item.LessonCode}:{translation.CultureCode}:Explanation");
                Assert.True(translation.KeyConceptsAndRules.Length >= 150, $"{item.LessonCode}:{translation.CultureCode}:KeyConceptsAndRules");
                Assert.True(translation.WorkedExamples.Length >= 90, $"{item.LessonCode}:{translation.CultureCode}:WorkedExamples");
                Assert.True(translation.StepByStepSolutions.Length >= 180, $"{item.LessonCode}:{translation.CultureCode}:StepByStepSolutions");
                Assert.True(translation.CommonMistakes.Length >= 120, $"{item.LessonCode}:{translation.CultureCode}:CommonMistakes");
                Assert.True(translation.QuickSummary.Length >= 110, $"{item.LessonCode}:{translation.CultureCode}:QuickSummary");
